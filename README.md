...
When you use the `derive_enum_tag macro` on an enum, it generates a marker struct for every variant of the enum, and also adds 
...
The marker structs are generated in their own module, which is named using the snake case version of the enum's name. See the examples below.
